...
src = cv2.imread("source_small.JPG", cv2.IMREAD_ANYCOLOR)
...
for i in range(num_of_pixels):
    ...
    D[i][i] = np.sum(W[i][:])
np.save("W_small.npy", W)
np.save("D_small.npy", D)

eigen_value, eigen_vector = sl.eig(D - W, D)
print("eigen value:", eigen_value)
np.save("eigen_value_small.npy", eigen_value)
np.save("eigen_vector_small.npy", eigen_vector)

min1 = min2 = math.inf
index1 = index2 = -1
for i in range(eigen_value.size):
    if eigen_value[i] >= min2:
        continue
    elif eigen_value[i] >= min1 and eigen_value[i] < min2:
        min2 = eigen_value[i]
        index2 = i
    else:
        min2 = min1
...
cv2.imwrite("mask_small.png", mask)